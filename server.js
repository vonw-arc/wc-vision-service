...
``,         `Water / sewer measurement rules (very important):`,         `- Identify the W (water meter pit) and S (sewer stub) symbols.`,         `- If an explicit utility line is drawn, use that route to determine the service path.`,         `- If NO utility line is drawn, you MUST assume the shortest reasonable straight-line path from the W or S symbol to the nearest logical foundation entry point for that unit.`,         `- A logical entry point means the closest spot on the foundation wall where that service would connect based on typical residential construction.`,         `- If a legible scale note (for example "1\\"=20'-0") or a graphic scale bar is present, you MUST attempt to estimate the water and sewer line lengths using that scale.`,         `- Use dimension text when available. Otherwise, measure using the scale and provide a reasonable estimate in feet, rounded to the nearest whole foot.`,         `- Always return ALL of the following fields, using empty strings ("") when truly unknown:`,         `     estimation_data.water_service_length_ft`,         `     estimation_data.water_service_length_method`,         `     estimation_data.sewer_service_length_ft`,         `     estimation_data.sewer_service_length_method`,         `- For <service>_length_method, use one of:`,         `     "dimension_text"              (length is clearly labeled with a dimension),`,         `     "scale_bar_approx"           (estimated via scale note or graphic scale bar),`,         `     "inferred_from_property_dims" (inferred from known lot dimensions and offsets),`,         `     "assumed_straight_path"      (no route drawn; assumed shortest reasonable path),`,         `     "unknown"                    (cannot determine length).`,         `- If you assumed a straight-line path, explicitly use "assumed_straight_path" and briefly explain in estimation_data.plot_grading_notes.`,
        ``,
...
``,         `General rules:`,         `- Prefer dimension text and explicit area callouts (e.g., "LOT AREA = 6000 SF", "BUILDING AREA = 1800 SF").`,         `- You MAY combine clearly labeled dimensions to infer a total length or area when it is straightforward.`,         `- Avoid wild guessing. When a value is not clearly stated or reasonably inferred from dimensions/scale, leave that field as an empty string ("").`,         `- If you approximate a value using the scale, keep it reasonable and mention the assumption in unusual_items, structural_notes, or estimation_data.plot_grading_notes.`,         `- Do NOT invent obviously unrealistic numbers.`,
      ]
